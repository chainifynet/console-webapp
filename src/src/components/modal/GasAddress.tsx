...
/**
 * TODO almost a copy past of Receive.tsx, refactor
 */
const GasAddress = (props: Props) => {
  const { assetId } = props;
  const { data, isLoading, error } = useGasStation(assetId);

  if (error) {
    return <Watermark text="Could not retrieve gas station data" />;
  }
  if (isLoading || !data) {
  ...
  return (
    <div className="text-center">
      ...
      <h2 className="text-2xl flex justify-center space-x-2 items-center">
        <Icon icon={icons.gasstation} />
        <div>Gas Station {data.assetId}</div>
      </h2>
      <div className="mt-4"></div>
